...
class Gemma3MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            hidden_states = 0.5 * hidden_states * (1+torch.tanh(torch.sqrt(2/torch.tensor(torch.pi))*(hidden_states + 0.044715*(hidden_states**3))))#.clone().detach()
            return self.down_proj(hidden_states.clone().detach() * self.up_proj(x))
...
class Gemma3RMSNorm(nn.Module):
    ...
    def _norm(self, x):
        if self.training:
            ...
            return x * (torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps).clone().detach())
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    if self.training:
        ...
        attn_output = torch.matmul(attn_weights.clone().detach(), value_states)

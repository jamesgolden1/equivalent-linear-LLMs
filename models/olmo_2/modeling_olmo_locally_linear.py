...
class Olmo2RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if not self.training:
            variance = hidden_states.pow(2).mean(-1, keepdim=True).clone().detach()
            hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon).clone().detach()
            return self.weight.clone().detach() * hidden_states.to(input_dtype)
...
class Olmo2MLP(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            return self.down_proj( (self.act_fn(self.gate_proj(x)).clone().detach()) * self.up_proj(x))

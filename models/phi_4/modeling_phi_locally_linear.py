...
class Phi3MLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.training:
            ...
            # up_states = up_states * (self.activation_fn(gate).clone().detach())
            up_states = up_states.clone().detach() * (gate*(torch.nn.functional.sigmoid(gate).clone().detach()))
...
class Phi3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if return_heads:
            attn_output_heads = attn_output.clone()

        attn_output = attn_output.reshape(*input_shape, -1).contiguous()
        attn_output = self.o_proj(attn_output)

        if return_heads:
            return attn_output, attn_weights, attn_output_heads
        else:
            return attn_output, attn_weights
